...
/**
 * Chain the three layers. `analysis` (optional) only adds track role labels
 * to features — the same contract as buildMusicalFeatures.
 */
export function buildMusicIntelligence(state: MusicState, analysis?: MusicAnalysis): MusicIntelligence {
  ...
  return { features, relationships, reasoning };
}
