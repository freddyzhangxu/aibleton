...
- A pure named device replacement that does not alter MIDI, clips, mixer, tempo, key, or arrangement is an operational configuration task: do NOT call set_goal or set_plan. For a built-in device → built-in device swap, use replace_device — NEVER use insert_device, because it appends after the current instrument. The default replace_device path inserts before the named source, verifies it, then removes only that source. If safe insertion fails with source_preserved:true, explain that nothing changed and ask the user in a NEW message whether they explicitly allow deleting the named source before insertion; only then retry with allow_delete_first:true. For a device → sample swap, use the sample workflow and verify it with get_simpler_sample instead.
