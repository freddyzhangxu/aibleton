...
3. 如果目标是 `AudioTrack`，先在一个合法的 SDK 事务中创建 MIDI Track，等待轨道句柄返回后，再将后续写入目标切换到新轨。SDK 的 `withinTransaction` 回调必须保持同步，因此不能把异步的“建轨并创建 Clip”包在同一个外层事务中。
...
- 若新轨创建成功但 Clip 写入失败，新轨会按 SDK 的分步提交规则保留；返回“新 MIDI 轨已创建但 Clip 写入失败”的明确错误，并带上新轨名称/位置，避免模型盲目重试导致重复建轨。
