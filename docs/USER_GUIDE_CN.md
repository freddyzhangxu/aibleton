...
Live 内置 webview 可能不能弹出原生文件选择框；在 Live 中优先用拖放或粘贴。若希望基于参考音频作分析，告诉 AI 本地 WAV / AIFF / MP3 / FLAC / OGG Vorbis / M4A（AAC 或 ALAC）的绝对路径以及要比较的段落；参考只用于分析差距和给出保守建议，不应要求复制一首现有作品。
